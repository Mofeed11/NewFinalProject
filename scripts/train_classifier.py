    ...
    for label in range(6):
        for _ in range(num_samples_per_class):
            if label == 0:  # VoIP (UDP, small packets, steady IAT)
                proto, sport, dport = 17, np.random.randint(10000, 60000), np.random.randint(10000, 60000)
                mean_sz, std_sz = np.random.normal(160, 10), np.random.uniform(0, 5)
                mean_iat, std_iat = np.random.normal(0.02, 0.005), np.random.uniform(0, 0.001)
                dur = np.random.uniform(10, 60)
            elif label == 1:  # Video (UDP/TCP, large packets, bursty)
                proto, sport, dport = np.random.choice([6, 17]), np.random.randint(10000, 60000), np.random.randint(10000, 60000)
                mean_sz, std_sz = np.random.normal(1000, 200), np.random.uniform(100, 300)
                mean_iat, std_iat = np.random.normal(0.01, 0.008), np.random.uniform(0.005, 0.01)
                dur = np.random.uniform(20, 120)
            elif label == 2:  # HTTP (TCP, port 80/443)
                proto, sport, dport = 6, np.random.randint(10000, 60000), np.random.choice([80, 443])
                mean_sz, std_sz = np.random.normal(500, 300), np.random.uniform(50, 400)
                mean_iat, std_iat = np.random.normal(0.1, 0.05), np.random.uniform(0.01, 0.1)
                dur = np.random.uniform(1, 30)
            elif label == 3:  # FTP (TCP, port 20/21)
                proto, sport, dport = 6, np.random.randint(10000, 60000), np.random.choice([20, 21])
                mean_sz, std_sz = np.random.normal(1400, 50), np.random.uniform(0, 20)
                mean_iat, std_iat = np.random.normal(0.005, 0.001), np.random.uniform(0, 0.002)
                ...
                mean_iat, std_iat = np.random.normal(0.05, 0.01), np.random.uniform(0, 0.01)
                dur = np.random.uniform(0.1, 2)
            elif label == 5:  # Background (UDP/TCP, random)
                proto, sport, dport = np.random.choice([6, 17]), np.random.randint(1000, 65000), np.random.randint(1000, 65000)
                mean_sz, std_sz = np.random.normal(800, 400), np.random.uniform(100, 500)
                mean_iat, std_iat = np.random.normal(0.5, 0.2), np.random.uniform(0.1, 0.5)
                dur = np.random.uniform(10, 300)

            # Derived fields
            pkts = int(max(1, dur / max(0.0001, mean_iat)))
            byts = int(pkts * mean_sz)
            pkt_size_ratio = np.random.uniform(0.8, 1.2)

            data.append([
                dur, pkts, byts, mean_sz, std_sz, 
                mean_iat, std_iat, proto, sport, dport, pkt_size_ratio
            ])
            labels.append(label)
